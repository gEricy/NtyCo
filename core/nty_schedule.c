...
/*
 *  brief: 创建协程调度器sche，相当于libco中的协程环境
 *         全局唯一，一个线程只有一个，线程中所有的协程都共用它
 *
 *  param [in] stack_size 调度器的协程栈大小，此处设为16*1024
 */
int nty_schedule_create(int stack_size) {
	...
	assert(pthread_setspecific(global_sched_key, sched) == 0);  // 绑定当前线程

    // 调度器：epoll
	sched->poller_fd = nty_epoller_create();

	nty_epoller_ev_register_trigger(); // ??? 监听 sched->eventfd 事件
...
static int nty_schedule_epoll(nty_schedule *sched) {
	...
	uint64_t usecs = nty_schedule_min_timeout(sched);  // 计算超时时间
...
void nty_schedule_run(void) {
	...
	while (!nty_schedule_isdone(sched)) {
		...
		// 3. wait rbtree
		nty_schedule_epoll(sched); // epoll_wait
